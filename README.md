...
**To add a new class's deck** (see [CONVENTIONS.md](CONVENTIONS.md) for the format and the quick-check mechanism in detail):

1. Author it with marimo (`marimo edit`), using the slides layout, and save it as `slides/ClassN/ClassN.py` alongside its `layouts/` and `custom.css`.
...
3. Add an entry to `_toc.yml` under the right class's `chapters:` (or add a new `caption:` part for a new class):
   ```yaml
   - url: https://bu-ek125.github.io/EK125-C1-DePasquale-Slides/DeckName/index.html
...
## https://BU-EK125.github.io/EK125-C1-DePasquale-Slides/intro.html
